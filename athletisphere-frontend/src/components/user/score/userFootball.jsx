...
useEffect(()=>{
    if (turf && turf.owner) {
    fetch(`http://localhost:8000/sports/footballuser?owner=${turf.owner}`).then((res)=>res.json()).then((result)=>{
    ...
    return(
        ...
        <div className="text-center">
            <div>Goal</div>
            <div>{getView.first_team_score}</div>
        </div>
        <div className="text-center">
            <div>Goal</div>
            <div>{getView.second_team_score}</div>
        </div>
        <div className="text-center">
